...
var OG = (function() {
  ...
  function toggleBillPaid(id){var i=findById(bills,id);if(i<0)return;var b=bills[i];if(!b.paid){b.paid=true;b.paidAt=Date.now();logChange("bills","paid",b.name);if(!b.recurring){renderBills();updateBillTotal();saveAll();setTimeout(function(){removeById(bills,id);renderBills();updateBillTotal();saveAll();},600);return;}}else{b.paid=false;b.paidAt=null;}renderBills();updateBillTotal();saveAll();}
